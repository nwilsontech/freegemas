...
void Board::generate(){
    ...
    do{
        ...
        for (int i = 0; i < 8; ++i)
        {
            for (int j = 0; j < 8; ++j)
            {
                squares[i][j] = static_cast<tSquare>(sf::Randomizer::Random(1,8));
                squares[i][j].mustFall = true;
                squares[i][j].origY = sf::Randomizer::Random(-7, -1);
    ...
    for(int x = 0; x < 8; ++x){
        ...
        for(int y = 0; y < 8; ++y){
            if(squares[x][y] == sqEmpty) {
                ...
                squares[x][y] = static_cast<tSquare> (sf::Randomizer::Random(1,8));
